...
func _show_level_select_if_set() -> void: 
	...
	if GAME_STATE_SCRIPT.get_levels_reached() <= 1 : return
	level_select_button.show()

func _show_continue_if_set() -> void:
	if GAME_STATE_SCRIPT.get_current_level_path().is_empty(): return
...
func _on_continue_game_button_pressed() -> void:
	GAME_STATE_SCRIPT.continue_game()
	load_game_scene()
